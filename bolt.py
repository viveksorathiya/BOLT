...
from include.pool import pool
from include.cloner import cloner
from include.prompt import prompt
from include.utils import *
from include.regex import rintels, rendpoint, rhref, rscript, rentropy
...
def extractor(url):
    ...
    for link in matches:
        ...
        if check_link(link, processed, files):
            if link[:4] == 'http':
    ...
    if args.regex and not supress_regex:
        extr(args.regex, response, supress_regex, custom)
    if api:
...
internal = set(removex(internal, args.exclude))
...
for level in range(crawl_level):
    ...
    links = removex(internal - processed, args.exclude)
    ...
    try:
        pool(extractor, links, thread_count)
...
if not only_urls:
    ...
    pool(jscanner, scripts, thread_count)
...
write(datasets, dataset_names, output_dir)
...
if args.dns:
    ...
    write([subdomains], ['subdomains'], output_dir)
